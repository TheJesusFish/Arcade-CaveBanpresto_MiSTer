...
`ifdef CAVE_ENABLE_DEBUG_OVERLAY
  reg        debugCtrlStartReg;
  reg        debugStartPulseReg;
  reg        debugDonePulseReg;
  reg        debugMissedStartPulseReg;
  reg [7:0]  debugStartCount;
  reg [7:0]  debugDoneCount;
  reg [7:0]  debugMissedStartCount;
  reg [9:0]  debugMaxSpriteThisFrame;
  reg [9:0]  debugMaxSpriteLastFrame;
  reg [15:0] debugTileWaitThisFrame;
  reg [15:0] debugTileWaitLastFrame;
  reg [15:0] debugFrameBufferWaitThisFrame;
  reg [15:0] debugFrameBufferWaitLastFrame;

  wire debugStartRequestPulse = io_ctrl_start & ~debugCtrlStartReg;
  wire debugMissedStart = debugStartRequestPulse & (stateReg != STATE_IDLE);
  wire debugTileWaitNow = (stateReg == STATE_PENDING) & ~effectiveRead;
  wire debugFrameBufferWaitNow = blitterBusy & ~io_frameBuffer_wait_n;
  wire debugReachedMetPortraitTail = debugMaxSpriteLastFrame >= 10'h222;

  always @(posedge clock) begin
    if (reset) begin
      debugCtrlStartReg <= 1'b0;
      debugStartPulseReg <= 1'b0;
      debugDonePulseReg <= 1'b0;
      debugMissedStartPulseReg <= 1'b0;
      debugStartCount <= 8'h00;
      debugDoneCount <= 8'h00;
      debugMissedStartCount <= 8'h00;
      debugMaxSpriteThisFrame <= 10'h000;
      debugMaxSpriteLastFrame <= 10'h000;
      debugTileWaitThisFrame <= 16'h0000;
      debugTileWaitLastFrame <= 16'h0000;
      debugFrameBufferWaitThisFrame <= 16'h0000;
      debugFrameBufferWaitLastFrame <= 16'h0000;
    end else begin
      debugCtrlStartReg <= io_ctrl_start;
      debugStartPulseReg <= acceptedFrameStart;
      debugDonePulseReg <= frameDone;
      debugMissedStartPulseReg <= debugMissedStart;

      if (acceptedFrameStart) begin
        debugStartCount <= debugStartCount + 8'h01;
        debugMaxSpriteThisFrame <= 10'h000;
        debugTileWaitThisFrame <= 16'h0000;
        debugFrameBufferWaitThisFrame <= 16'h0000;
      end else begin
        if (spriteCounter > debugMaxSpriteThisFrame)
          debugMaxSpriteThisFrame <= spriteCounter;
        if (debugTileWaitNow & ~&debugTileWaitThisFrame)
          debugTileWaitThisFrame <= debugTileWaitThisFrame + 16'h0001;
        if (debugFrameBufferWaitNow & ~&debugFrameBufferWaitThisFrame)
          debugFrameBufferWaitThisFrame <= debugFrameBufferWaitThisFrame + 16'h0001;
      end

      if (debugMissedStart)
        debugMissedStartCount <= debugMissedStartCount + 8'h01;

      if (frameDone) begin
        debugDoneCount <= debugDoneCount + 8'h01;
        debugMaxSpriteLastFrame <= debugMaxSpriteThisFrame;
        debugTileWaitLastFrame <= debugTileWaitThisFrame;
        debugFrameBufferWaitLastFrame <= debugFrameBufferWaitThisFrame;
      end
    end
  end

  wire [7:0] debugFlags = {
    debugReachedMetPortraitTail,
    stateReg != STATE_IDLE,
    debugFrameBufferWaitNow,
    debugTileWaitNow,
    debugMissedStartPulseReg,
    frameReadyReg,
    debugDonePulseReg,
    debugStartPulseReg
  };
`endif
  ...
  SpriteBlitter blitter (
    ...
    .io_config_bits_sprite_code      (spriteReg_code),
...
`ifdef CAVE_ENABLE_DEBUG_OVERLAY
  assign io_debug = {
    ...
    debugTileWaitLastFrame[7:0],
    debugMissedStartCount,
    debugDoneCount,
    debugStartCount,
    {6'h00, debugMaxSpriteLastFrame[9:8]},
    debugMaxSpriteLastFrame[7:0],
    debugFlags
  };
`else
