...
module CaveDebugOverlay(
  ...
  always @* begin
    ...
    case (debugTextCol)
      3'd0: begin
        if (io_debug_view == 3'd6) begin
          case (debugTextRow)
            3'd0, 3'd1: debugSoundChar = DEBUG_CH_S;  // STA/SPR
            3'd2: debugSoundChar = DEBUG_CH_M;        // MSN
            3'd3: debugSoundChar = DEBUG_CH_T;        // TLR
            3'd4, 3'd5: debugSoundChar = DEBUG_CH_H;  // H0/H1
            3'd6: debugSoundChar = DEBUG_CH_P;        // PCL
            default: debugSoundChar = DEBUG_CH_M;     // MSK
          endcase
        end
        else if (io_debug_view == 3'd5) begin
          case (debugTextRow)
      ...
      3'd1: begin
        if (io_debug_view == 3'd6) begin
          case (debugTextRow)
            3'd0: debugSoundChar = DEBUG_CH_T;  // STA
            3'd1: debugSoundChar = DEBUG_CH_P;  // SPR
            3'd2: debugSoundChar = DEBUG_CH_S;  // MSN
            3'd3: debugSoundChar = DEBUG_CH_L;  // TLR
            3'd4: debugSoundChar = 6'd0;        // H0
            3'd5: debugSoundChar = 6'd1;        // H1
            3'd6: debugSoundChar = 6'd12;       // PCL
            default: debugSoundChar = DEBUG_CH_S;// MSK
          endcase
        end
        else if (io_debug_view == 3'd5) begin
          case (debugTextRow)
      ...
      3'd2: begin
        if (io_debug_view == 3'd6) begin
          case (debugTextRow)
            3'd0: debugSoundChar = 6'd10;       // STA
            3'd1: debugSoundChar = DEBUG_CH_R;  // SPR
            3'd2: debugSoundChar = DEBUG_CH_N;  // MSN
            3'd3: debugSoundChar = DEBUG_CH_R;  // TLR
            3'd4, 3'd5: debugSoundChar = DEBUG_CH_SPACE;
            3'd6: debugSoundChar = DEBUG_CH_L;  // PCL
            default: debugSoundChar = DEBUG_CH_K;// MSK
          endcase
        end
        else if (io_debug_view == 3'd5) begin
          case (debugTextRow)
  ...
  wire       debugTextGlyphOn =
    debugPanel &
    ((io_debug_view == 3'd5) | (io_debug_view == 3'd6) | (io_debug_view == 3'd7)) &
    (debugTextCol < 3'd6) &
